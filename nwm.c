...
/* https://en.wikipedia.org/wiki/Terry_A._Davis#/media/File:Terry_A._Davis_(cropped).jpg */
...
static Bool mvpred(Display*, XEvent*, XPointer);
static C   *nextt(C*);
static void pop(C*);
...
aph(C *c, int *x, int *y, int *w, int *h, int i) {
	...
	*h = MAX(1, *h);
	if (i) {
		if (*x > sw)            *x = sw - (*w + bw2); /* A bug once lived here. */
		if (*x + *w + bw2 < 0) *x = 0;
		if (*y + *h + bw2 < 0) *y = 0;
	} else {
...
fc(C *c) {
	...
	if (c) {
		...
		XSetInputFocus(d, r, RevertToPointerRoot, ltime);
...
monocle(void) {
	...
	for (c = nextt(cs); c; c = nextt(c->next))
		rs(c, wx, wy, MAX(1, ww - (c->bw << 1)), MAX(1, wh - (c->bw << 1)), 0);
}

/* Matches the same events mv()'s drag loop always waited for, plus a
 * KeyPress for a bare-MODKEY digit 1-9 (workspace switch while dragging).
 * Anything else is left in the queue untouched, exactly as before. */
static Bool
mvpred(Display *dpy, XEvent *e, XPointer arg) {
	(void)dpy; (void)arg;
	switch (e->type) {
	case ButtonPress:
	case ButtonRelease:
...
mv(const A *arg) {
	...
	do {
		...
		if (ev.type == ConfigureRequest || ev.type == Expose || ev.type == MapRequest)
			...
			/* Bare-MODKEY digit while dragging: send the window to that
			 * workspace and follow it there, keeping its exact x/y/w/h. */
			unsigned int ti = (unsigned int)(XLookupKeysym(&ev.xkey, 0) - XK_1);
			if (ti < LEN(tags)) {
				unsigned int m = 1u << ti;
				c->isfloating = c->oldstate = 1;
				c->tags = m;
				if (m != ts[sg]) { sg ^= 1; ts[sg] = m; }
				needar = 0;
				ar();
			}
		}
...
static void pop(C *c) { detach(c); c->next = cs; cs = c; fc(c); ar(); }
...
setup(void) {
	...
	if (!XAllocNamedColor(d, cmap, colnb, &xc, &exact)) die("nwm: cannot allocate color");
	nborder = xc.pixel;
	if (!XAllocNamedColor(d, cmap, colsb, &xc, &exact)) die("nwm: cannot allocate color");
	sborder = xc.pixel;
	if (!XAllocNamedColor(d, cmap, colub, &xc, &exact)) die("nwm: cannot allocate color");
	uborder = xc.pixel;
...
tile(void) {
	...
	for (i = 0, c = nextt(cs); c; c = nextt(c->next), i++) {
		...
		if (i < nmv) {
			y0 = wy + g + (int)i * (mch + g);
			rs(c, wx + g, y0, MAX(1, mw - bw2), MAX(1, mch - bw2), 0);
		} else {
			y0 = wy + g + (int)(i - nmv) * (sch + g);
			rs(c, wx + (nmv ? mw + 2*g : g), y0,
				MAX(1, (nmv ? ww - mw - 3*g : ww - 2*g) - bw2),
				MAX(1, sch - bw2), 0);
		}
	}
}
